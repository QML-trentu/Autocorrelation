# -*- coding: utf-8 -*-
"""
Created on Mon Aug  3 17:24:08 2026

@author: carlo
"""
"""
Plot_g2_at_iterations.py

Publication-quality figure: g2(tau) for ONE real experimental file, shown
side-by-side at several specified iter_# columns (integration times) - the
same "sparse to clean" panel convention used in prior literature (e.g.
...
SETTINGS = {
    ...
    "save_path": "g2_at_iterations.png",
    "dpi": 300,
...
def make_figure(settings):
    ...
    fig, axes = plt.subplots(
        1, n_panels,
        figsize=(settings["figsize_per_panel"][0] * n_panels, settings["figsize_per_panel"][1]),
        sharey=False,
    )
    if n_panels == 1:
        axes = [axes]
    ...
    for ax, it, label in zip(axes, iters, panel_labels):
        col_idx = it - 1
        ...
        ax.set_title(rf"({label}) $t_{{\mathrm{{int}}}}$ = {times_s[col_idx]:g} s", fontsize=11)
